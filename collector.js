...
(function () {
  ...
  // Config
  const CONFIG = {
    // Use a relative path so this works behind an Apache reverse proxy
  ...
  function areImagesEnabled() {
    try {
      // If the browser blocks images, this still creates an element
      // Most browsers allow data URLs, so return true
      const img = new Image();
  ...
  function collectPerformanceData() {
    ...
    // Fall back to legacy timing if Navigation Timing Level 2 is not available
  ...
  function sendData() {
    ...
    // Prefer sendBeacon when available; non-blocking and can survive page close
